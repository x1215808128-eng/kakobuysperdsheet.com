    ...
    <article className="px-4 py-16">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-4 text-zinc-600">
          If you&apos;re using <strong>Kakobuy Spreadsheet</strong> to find
          products, these guides answer common questions about payments,
          shipping, QC, and refunds.
        </p>
        ...
        <div className="mt-10 space-y-8">
          {GUIDE_SECTIONS.map((section, i) => (
            <section key={section.title}>
              ...
              <p className="mt-2 text-zinc-600">{section.body}</p>
            </section>
          ))}
        </div>
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          {GUIDE_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full border border-zinc-200 px-4 py-2 text-sm font-medium hover:border-orange-300 hover:text-orange-600"
            >
              → {link.label}
            </Link>
          ))}
